...
export const PatientDashboard: React.FC = () => {
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 overflow-hidden">
      ...
      <div className="fixed top-20 -left-20 w-80 h-80 bg-teal-300 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob"></div>
      <div className="fixed -bottom-20 -right-20 w-80 h-80 bg-indigo-300 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob" style={{ animationDelay: '2s' }}></div>
      <div className="fixed top-1/2 -left-10 w-40 h-40 bg-emerald-200 rounded-full mix-blend-multiply filter blur-2xl opacity-20 animate-blob" style={{ animationDelay: '4s' }}></div>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 animate-fade-in-up">
           <div>
              <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight">
                Hello, <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-600 to-indigo-600">{currentPatientName.split(' ')[0]}</span>
              </h1>
              <p className="text-slate-500 mt-2 font-medium text-lg">How are you feeling today?</p>
           </div>
        </div>

        {/* --- MAIN FEATURE SECTION --- */}
        <section className="bg-white/90 backdrop-blur-sm rounded-[2.5rem] p-6 sm:p-10 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.08)] border border-white/60 relative overflow-visible animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
            
            <h2 className="text-xl font-bold text-slate-800 mb-8 flex items-center gap-3">
               <span className="w-1.5 h-7 bg-gradient-to-b from-teal-500 to-emerald-500 rounded-full shadow-sm shadow-teal-500/20"></span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div className={`relative rounded-[2rem] border-2 transition-all duration-300 flex flex-col items-center justify-center p-6 gap-3 min-h-[160px] group ${recordedAudio ? 'bg-indigo-50/80 border-indigo-200 shadow-lg shadow-indigo-500/10' : 'bg-slate-50/50 border-slate-100 hover:border-teal-300 hover:bg-teal-50/40 hover:scale-[1.02] hover:shadow-xl hover:shadow-teal-500/5'}`}>
                    <div className="scale-125 transform transition-transform group-hover:scale-[1.35]">
                         <AudioRecorder onAudioCaptured={setRecordedAudio} />
                    </div>
                    <div className="text-center mt-2">
                        <p className={`text-sm font-bold ${recordedAudio ? 'text-indigo-700' : 'text-slate-600'}`}>
                           {recordedAudio ? 'Voice Captured' : 'Record Symptoms'}
                        </p>
                        {recordedAudio && (
                           <button onClick={() => setRecordedAudio(null)} className="text-[10px] text-red-500 font-bold uppercase hover:underline mt-1">Remove recording</button>
                        )}
                    </div>
                </div>
                ...
                <button 
                  ...
                  className={`relative rounded-[2rem] border-2 transition-all duration-300 flex flex-col items-center justify-center p-6 gap-4 min-h-[160px] group ${selectedImage || selectedPdf ? 'bg-teal-50/80 border-teal-200 shadow-lg shadow-teal-500/10' : 'bg-slate-50/50 border-slate-100 hover:border-teal-300 hover:bg-teal-50/40 hover:scale-[1.02] hover:shadow-xl hover:shadow-teal-500/5'}`}
                >
                   {selectedImage || selectedPdf ? (
                      <>
                        <div className="h-14 w-14 bg-gradient-to-br from-teal-100 to-emerald-100 text-teal-600 rounded-2xl flex items-center justify-center text-3xl shadow-sm border border-teal-200 group-hover:scale-110 transition-transform">
                         ...
                         <div className="text-center">
                           ...
                           <p className="text-[10px] text-teal-500/80 font-bold uppercase tracking-widest mt-1">Ready for Analysis</p>
                        </div>
                        <div 
                          className="absolute top-4 right-4 text-teal-400 hover:text-red-500 p-2 transition-all bg-white/50 rounded-full hover:bg-white shadow-sm" 
                          onClick={(e) => { 
                            e.stopPropagation(); 
                            setSelectedImage(null); 
                            setSelectedPdf(null); 
                      ...
                      <>
                        <div className="h-14 w-14 bg-white text-teal-600 rounded-2xl flex items-center justify-center text-2xl shadow-sm border border-slate-200 group-hover:bg-teal-50 group-hover:scale-110 transition-all">
                        ...
                        <div className="text-center">
                           ...
                           <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">PDF or Medical Image</p>
                        </div>
                      </>
                   )}
                </button>
                ...
                <div className="relative group">
                    ...
                    >
                       ...
                       <div className="text-center">
                          ...
                          <p className={`text-[10px] font-bold uppercase tracking-widest mt-1 ${selectedDoctorId ? 'text-indigo-600' : 'text-slate-400'}`}>
                             {selectedDoctorId ? 'Expert Review Request' : 'Human Oversight Optional'}
                          </p>
                       </div>
                    </button>
            ...
            <div className="mt-10">
               <Button 
                 ...
                 fullWidth
                 className="py-5 rounded-[1.4rem] text-base shadow-[0_20px_40px_rgba(13,148,136,0.25)] font-black tracking-widest uppercase hover:shadow-[0_25px_50px_rgba(13,148,136,0.35)] transition-all hover:scale-[1.01] active:translate-y-0.5"
               >
                 {isProcessing ? (
                    <span className="flex items-center justify-center gap-3">
                       <svg className="animate-spin h-5 w-5 text-white" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                       AI Sathi is Processing Information...
                    </span>
        ...
        <section className="animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
           <div className="flex items-center justify-between mb-8">
             <h2 className="text-xl font-black text-slate-800 flex items-center gap-3">
                <span className="w-2 h-2 rounded-full bg-indigo-500 shadow-[0_0_15px_rgba(79,70,229,0.5)] animate-pulse"></span>
                Recent Assessments
             </h2>
             <span className="text-xs font-bold text-slate-400 uppercase tracking-widest bg-white/50 px-3 py-1 rounded-full border border-slate-100">{totalReports} Records</span>
           </div>
           ...
           {reports.length === 0 ? (
             ...
             <div className="space-y-8">
               {reports.map((report, idx) => {
                 ...
                 return (
                 <Card key={report.id} className="border-0 bg-white/70 backdrop-blur-sm shadow-[0_10px_30px_-15px_rgba(0,0,0,0.05)] hover:shadow-[0_25px_50px_-12px_rgba(0,0,0,0.12)] hover:-translate-y-1.5 transition-all duration-500 p-0 overflow-hidden rounded-[2.5rem] animate-fade-in-up" style={{ animationDelay: `${idx * 0.1 + 0.3}s` }}>
                   {/* Card Header */}
